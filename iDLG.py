...
def main():
    ...
    for idx_net in range(num_exp):
        initnet = LeNet(channel=channel, hideen=hidden, num_classes=num_classes)
        initnet.apply(weights_init)

        print('running %d|%d experiment' % (idx_net, num_exp))
        initnet = initnet.to(device)
        idx_shuffle = np.random.permutation(len(dst))

        maskedmodel = deepcopy(initnet)
        unmaskedmodel = deepcopy(initnet)

        for name, param in maskedmodel.named_parameters():
            print(name)
            print(param.shape)
            print(param.data.numpy().round(4))
            param.data = torch.from_numpy(param.data.numpy().round(4))
            print(param.data)

        for name, param in unmaskedmodel.named_parameters():
            print(name)
            print(param.shape)
            #to normal model
            # param.data = torch.from_numpy(param.data.numpy().round(4))
            #to masked model
            param.data = torch.from_numpy(param.data.numpy().round(4) + k)
            print(param.data)

        models = {'unmasked': unmaskedmodel, 'masked': maskedmodel}
                ...
                for iters in range(Iteration):
                    ...
                    if iters % int(Iteration / 30) == 0:
                        ...
                        for imidx in range(num_dummy):
                            ...
                            if method == 'DLG':
                                plt.savefig(
                                    '%s/DLG_on_%s_%05d_%s.png' % (save_path, imidx_list, imidx_list[imidx], name))
                                plt.close()
                            elif method == 'iDLG':
                                plt.savefig(
                                    '%s/iDLG_on_%s_%05d_%s.png' % (save_path, imidx_list, imidx_list[imidx], name))
                                plt.close()
